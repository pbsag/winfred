...
```{r load_network}
network <- readShapeLines("data/networkdevelopment/trueshape_simplified.shp", 
                          proj4string = CRS("+init=epsg:3968")) %>%
  spTransform(., CRS("+init=epsg:4326"))

new_values <- read.dbf("data/networkdevelopment/WF_12A_VDOT_UPDATE.dbf",
                       as.is = TRUE)

network@data <- network@data %>%
  select(A, B) %>%
  left_join(new_values, by = c("A", "B"))
...
The maps below show the speed limits and lanes on this network, with the TAZ 
boundaries available as well.


### Lanes
```{r network_leaflet_setup}
...
pal_lanes <- colorFactor(
  palette = "Dark2",
  domain = factor(network$LANES)
)
...
leaflet(network) %>%
  ...
  addLayersControl(
    overlayGroups = c("TAZ", "Lanes"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  addLegend(position = "bottomright", pal = pal_lanes, values = ~LANES)
...
pal_speeds <- colorFactor(
  palette = "Set1",
  domain = factor(network$POST_SPD)
)
...
leaflet(network) %>%
  ...
  addPolylines(group = "Speed Limits", color = ~pal_speeds(POST_SPD), 
               popup = ~basic_network(network))  %>%
  addLayersControl(
    overlayGroups = c("TAZ", "Speed Limits"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  addLegend(position = "bottomright", pal = pal_speeds, values = ~POST_SPD)
```
